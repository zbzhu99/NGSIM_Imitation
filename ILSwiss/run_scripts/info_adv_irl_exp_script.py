...
from rlkit.torch.common.networks import FlattenConditionalMlp
from rlkit.torch.common.policies import ConditionalReparamTanhMultivariateGaussianPolicy
from rlkit.torch.algorithms.sac.conditional_sac_alpha import ConditionalSoftActorCritic
from rlkit.torch.algorithms.adv_irl.disc_models.simple_disc_models import MLPDisc
from rlkit.torch.algorithms.info_adv_irl.posterior_models.posterior_trainer import (
    PosteriorTrainer,
)
from rlkit.torch.algorithms.info_adv_irl.posterior_models.simple_posterior_models import (
    MlpPosterior,
)
...
def experiment(variant):
    ...
    with open(train_split_path, "rb") as f:
        ...
        print(f"train_vehicle_num: {sum([len(x) for x in train_vehicles.values()])}")
    ...
    with open(eval_split_path, "rb") as f:
        ...
        print(f"eval_vehicle_num: {sum([len(x) for x in eval_vehicles.values()])}")
    ...
    for agent_id in env.agent_ids:
        ...
        if policy_id not in policy_trainer_n:
            ...
            policy_trainer = ConditionalSoftActorCritic(
                policy=policy,
                qf1=qf1,
                qf2=qf2,
                vf=vf,
            ...
            posterior_trainer = PosteriorTrainer(
                posterior_model=posterior_model,
                **variant["posterior_params"],
            )

            policy_trainer_n[policy_id] = policy_trainer
            policy_n[policy_id] = policy
            disc_model_n[policy_id] = disc_model
            posterior_trainer_n[policy_id] = posterior_trainer
        else:
    ...
    algorithm = InfoAdvIRL(
        ...
        posterior_trainer_n=posterior_trainer_n,
